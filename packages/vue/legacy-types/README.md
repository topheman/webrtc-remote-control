...
from the TypeScript sources are still assignable to these, so an option or a
hook result that narrowed during the port fails `vp check` instead of reaching a
consumer. The same arrangement guards `packages/core` and `packages/react`.
...
option is passed straight to core's `prepareUtils`, which expects the wrapper
