...
`packages/core` and `packages/react` are TypeScript; `packages/vue` and the demo are not
yet. The declarations under `dist` are generated by tsdown from the sources (`dts: true` in
each package's `pack` block), so there is nothing to hand-edit and nothing that can drift.
...
`**/*.tsx`; the vue package and the demo join it as each is ported.
...
- There are `console.log` calls shipped in `vue/hooks.js` and `vue/Provider.js`. A
  published package should not write to the console of the application embedding it unless
  that application opted in, so these are going: core's and react's went with their
  TypeScript ports and vue's goes with its own. `no-console` is set to `warn` in
  `vite.config.ts` until then, at which point it becomes
...
- `remote ? masterPeerId : undefined` tests an imported module, so it is always truthy. It
  was probably meant to be `mode === "remote"`. Harmless today, because master mode
  guarantees `masterPeerId` is undefined and master's `bindConnection` ignores a second
  argument. It survives only in `vue/Provider.js`: the react port had to split the call in
  two, because the master and remote sides take different arguments and no single typed
