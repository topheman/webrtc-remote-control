...
`packages/core` is TypeScript; `packages/react`, `packages/vue` and the demo are not yet.
The declarations under `dist` are generated by tsdown from the sources (`dts: true` in the
package's `pack` block), so there is nothing to hand-edit and nothing that can drift.

The hand-written declarations the package used to ship are **frozen** under
`packages/core/legacy-types/`. Nothing imports them at runtime and they are not published;
...
- There are `console.log` calls shipped in `vue/hooks.js`, `vue/Provider.js` and
  `react/Provider.jsx`. A published package should not write to the console of the
  application embedding it unless that application opted in, so these are going: core's
  went with its TypeScript port and the rest go with theirs. `no-console` is set to `warn`
  in `vite.config.ts` until then, at which point it becomes
...
- `remote ? masterPeerId : undefined` in both Provider files tests an imported module, so
  it is always truthy. It was probably meant to be `mode === "remote"`. Harmless today.
- `packages/react/src/Provider.jsx` lists `utils` in its `useEffect` dependencies, and
  `utils` is rebuilt every render, so the effect re-runs every render.
